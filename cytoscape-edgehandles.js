...
/***/ (function (module, exports, __webpack_require__) {
      ...
      function makeEdges() {
        ...
        var source2target = cy.add(getEleJson({
          ...
          data: {
            // mofication interne zouanah
            id: source.data().label + target.data().label,
            source: source.id(),
            target: target.id(),
            // value: 0,
            edgeValue: 0,
          ...
          cy.batch(function () {
            ...
            ghostEdge = cy.add(assign({}, ghostEdgeParams, {
              ...
              data: assign({}, ghostEdgeParams.data, {
                ...
                target: ghostNode.id(),
                // modification code interne zouanah
                edgeValue: ''
              }),
      ...
      function preview(target) {
        ...
        var applyPreview = function applyPreview() {
          ...
          if (options.preview) {
            ...
            // modification code interne zouanah
            let idEdgeArr = [];
            _this.cy.edges().forEach(element => {
              if (element.data().id.length < 10) {
                idEdgeArr = [...idEdgeArr, element.data().id];
              }
            });
            if (idEdgeArr.includes(source.data().label + target.data().label)) {
              /***********************
               * Afficher un message
               */
              console.log('Arc déjà creer');
              // alert('Arc déjà creer');
            }
            else {
              // chainage du sommet et arc
              // ICI C'est le coeur de preview
              _this.makePreview();
            }
